 ...
 */
static void PCA9685_WriteByte(uint32_t fd, UBYTE reg, UBYTE value)
{
    i2cWriteByteData(fd, reg, value);
 ...
 */
static UBYTE PCA9685_ReadByte(uint32_t fd, UBYTE reg)
{
    int ref;
    ref = i2cReadByteData(fd, reg);
    return ref;
 ...
 */
static void PCA9685_SetPWM(uint32_t fd, UBYTE channel, UWORD on, UWORD off)
{
 ...
 * PCA9685_Init(0x40);
 */
uint32_t PCA9685_Init(char addr)
{
    uint32_t fd;
    fd = i2cOpen(1, addr, 0);
    i2cWriteByteData(fd, MODE1, 0x00);
    return fd;
 ...
 */
void PCA9685_SetPWMFreq(uint32_t fd, UWORD freq)
{
 ...
 */
void PCA9685_SetPwmDutyCycle(uint32_t fd, UBYTE channel, UWORD pulse)
{
 ...
 */
void PCA9685_SetLevel(uint32_t fd, UBYTE channel, UWORD value)
{
    if (value == 1)
